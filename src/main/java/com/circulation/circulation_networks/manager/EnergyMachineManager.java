...
public final class EnergyMachineManager {
    ...
    public void onBlockPositionReady(Level world, BlockPos position) {
        submitPositionLifecycle(world, position, MachineLifecyclePositionIndex.Action.READY, null);
    }

    public void onManagerUnavailableAtPosition(Level world,
                                               BlockPos position,
    ...
    public synchronized RegistrationResult addMachineNode(CFNBlockEntityEx blockEntity) {
        ...
        INode directNode = Objects.requireNonNull(machineNode.getNode(), "Machine node returned null node");
        IEnergyHandler directHandler = Objects.requireNonNull(
            machineNode.getEnergyHandler(), "Machine node returned null handler"
        );
        attachMachineNodeMemberships(blockEntity, directNode);
        MachineHandlerRuntime runtime = blockEntity.cfn_getMachineHandlerRuntime();
        MachineRoute route = machineRoutes.get(blockEntity);
        if (runtime != null && runtime.directHandler() == directHandler && runtime.isBound()
        ...
        try {
            ...
            MachineHandlerRuntime replacement = new MachineHandlerRuntime(
                blockEntity, directHandler, null, null, dimensionId, packedPosition
            );
            try {
    ...
    private boolean isCurrentBlockEntity(CFNBlockEntityEx blockEntity) {
        ...
        Object currentBlockEntity = world.getBlockEntity(position);
        return currentBlockEntity == blockEntity;
    }
    ...
    void initGrid(Collection<NetworkManager.GridEntry> entries) {
        Reference2ObjectMap<Level, LongSet> bootstrapChunks = new Reference2ObjectOpenHashMap<>();
        for (var entry : entries) {
            ...
            for (INode node : entry.grid().getNodes()) {
                ...
                LongSet chunksCovered = indexSupplyNodeScope(energySupplyNode, dim.hashCode());
    ...
    private void bootstrapLoadedChunks(Level world, LongSet chunkCoordinates) {
    ...
    private boolean reconcileLoadedMachinePosition(int dimensionId,
        ...
        var nativeBlockEntity = world.getBlockEntity(position);
    ...
    private void submitPositionLifecycle(Level world,
                                         BlockPos position,
    ...
    private void submitLifecycle(Level world,
                                 BlockPos position,
    ...
    public void removeNode(INode node) {
        ...
        if (node instanceof IEnergySupplyNode removedNode) {
            ...
            int dimId = removedNode.getDimensionId().hashCode();
    ...
    private IEnergyHandler.EnergyType getOverride(@Nullable EnergyTypeOverrideManager overrideManager,
        ...
        Long2ObjectMap<IEnergyHandler.EnergyType> dimOverrides = overrideManager.getOverridesForDim(dimensionKey);
    ...
    private record PositionLifecycleCommand(int dimensionId, long packedPosition,
                                            MachineLifecyclePositionIndex.Action action,
                                            @Nullable IEnergyHandlerManager excludedManager, int resolutionAttempts) {
    ...
    private record PositionLifecycleKey(int dimensionId, long packedPosition,
                                        MachineLifecyclePositionIndex.Action action) {
            ...
            public boolean equals(Object other) {
                ...
                if (!(other instanceof PositionLifecycleKey(
                    int id, long position, MachineLifecyclePositionIndex.Action action1
                ))) {
                    return false;
                }
    ...
    private final class MachineRoute implements MachineBindingIndex.Route {
        ...
        private void rebuildRouteConfiguration() {
            ...
            routeEnabled = routed && world != null
                ...
                && !CirculationShielderManager.INSTANCE.isBlockedByShielder(dimensionKey, position);
            if (!routeEnabled) {
                account = null;
                clearSlots();
                return;
        ...
        private void configureSlot(IGrid grid, MachineTransferSlot slot) {
            ...
            IEnergyHandler.EnergyType override = machineNode
                ? null
                : getOverride(EnergyTypeOverrideManager.get(), dimensionKey, packedPosition);
    ...
    private record RegistrationRollback(Throwable failure, boolean failed) {
